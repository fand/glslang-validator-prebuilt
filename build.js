...
p(rimraf)(tempDir)
  .then(() =>
    run(mkdirp(tempDir), `Failed to create temporal directory: '${tempDir}'`)
  )
  .then(() =>
    run(download(url, tempDir), `Failed to download the binary to: ${tempDir}`)
  )
  .then(() =>
    run(extract(zipPath, tempDir), `Failed to extract zip file: '${zipPath}'`)
  )
  .then(() =>
    run(
      p(rimraf)(dstBinDir),
      `Failed to clean up bin directory: '${dstBinDir}'`
    )
  )
  .then(() =>
    run(
      mkdirp(dstBinDir),
      `Failed to create temporal directory: '${dstBinDir}'`
    )
  )
  .then(() =>
    run(
      p(fs.copyFile)(unzippedBinPath, dstBinPath),
      `Failed to copy binary to: '${dstBinPath}'`
    )
  )
  .then(() =>
    run(
      p(fs.chmod)(dstBinPath, "755"),
      `Failed to chmod binary: '${dstBinPath}'`
    )
  )
  .then(() => {
